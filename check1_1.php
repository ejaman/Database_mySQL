...
<body id="top" data-spy="scroll" data-target=".navbar-collapse" data-offset="50">
  ...
  <section>
     ...
     <h3 align="left"><h3><?php echo $inst_code  ?> 의 강사 정보를 조회합니다</h3></br>
     <div class="table-responsive">
         <table class="table table-bordered table-striped">
          ...
          <th width="20%">스케줄</th>
            ...
            $query = "SELECT * FROM cultureland.instructor
                      join course using (inst_code) 
                      where inst_code = '$inst_code' ";

            $exec = mysqli_query($con,$query);
            while($row = mysqli_fetch_array($exec)){
            echo '<tr><td>' . $row[ 'admin_code' ] . '</td><td>'. $row[ 'inst_name' ] . '</td>
            <td>' . $row[ 'inst_phone' ] . '</td><td>' . $row[ 'inst_email' ] .  '</td>
            <td>'.$row['inst_address'].'</td><td>'. $row[ 'inst_career' ] . '</td><td>'. $row[ 'inst_degree' ] . '</td>
            <td>'. $row[ 'course_code' ] . '</td><td>'. $row[ 'course_name' ] .  '</td><td>'. $row[ 'course_fee' ] . '</td>
